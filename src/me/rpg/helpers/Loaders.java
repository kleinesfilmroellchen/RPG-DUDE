...
import me.rpg.core.objects.Enemy;
import me.rpg.core.objects.Room;
...
public class Loaders {
	 ...
	 */
	public static Room[][][] loadRoomsFromMap(String absolutePath) throws JSONException, FileNotFoundException, IOException, NumberFormatException, ArrayIndexOutOfBoundsException {
		...
		if (!map.getString("compatible-version").equalsIgnoreCase(GameConst.VERSION)) {
			System.err.println("Fehler: Kartendatei ist nicht mit Spiel kompatibel.\nBitte verwende eine Karte, die dieselbe Version wie dieses Spiel (" + GameConst.VERSION + ") aufweist.");
			System.exit(1);
		}
		...
		for (int i = 0; i < roomDescriptions.length(); ++i) {
			...
			roomArrayMatrix[posArray.getInt(2)][posArray.getInt(0)][posArray.getInt(1)].roomName = descObj.getString("name");
			roomArrayMatrix[posArray.getInt(2)][posArray.getInt(0)][posArray.getInt(1)].roomDescription = descObj.getString("description");
		}
	...
	/**
	 * Reads a JSON (Java Script Object Notation) file and creates enemies from the data found in the file. All enemies it can find are returned.
	 * @param absolutePath The absolute Path to the file.
	 * @return All Entities, parsed from the JSON.
	 * @throws JSONException if the JSON contains syntactical errors at an unhandled position in the method.
	 ...
	 */
	public static ArrayList<Enemy> getEnemies(String absolutePath) throws JSONException, ClassCastException, IOException {
		...
		for (int i = 0; i < array.length(); i++) {
			...
			try {
				...
				System.err.println("JSON file " + absolutePath + " contains an unreadable enemy. Make sure your file has all necessary properties for all enemies and has correct syntax.");
				je.printStackTrace();
			}
		}
		return returner;
	 ...
	 * @return The enemy, if there is one with the given id. Otherwise, {@code null} is returned.
	 ...
	 * @param dict The enemy dictionary which is used to create instances by cloning the respective enemy in the dictionary.
	 * @return
	 * @throws IOException if something with I/O goes wrong.
	 * @throws FileNotFoundException if the file cannot be found.
	 * @throws JSONException if the JSON is unreadable or an undefined enemy is present in the map.
	 */
	public static ArrayList<Enemy> enemiesFromMap(String absolutePath, ArrayList<Enemy> dict) throws JSONException, FileNotFoundException, IOException {
		...
		for (int i = 0; i < enemies.length(); i++) {
			...
			Optional<Enemy> reference = dict.stream().filter(element -> element.id.equalsIgnoreCase(enemyObj.getString("id"))).findFirst();
			//if the optional contains a non-null value we found something in the dictionary
			if (reference.isPresent()) {
				...
				returnEnemies.get(returnEnemies.size()-1).setCoords(posArray.getInt(0), posArray.getInt(1), posArray.getInt(2));
			} else {
				//this should not happen: throw exception
				throw new JSONException("The enemy " + enemyObj.getString("id") + " is not specified in the enemies.json file.\n"
