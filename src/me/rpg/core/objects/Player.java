...
import me.rpg.core.objects.items.Abillity;
import me.rpg.helpers.Functionality;
import utils.BetterMath;

/**
 * The Player object.
 * Currently only instantiated once.
 * @author DJH
...
public class Player extends Entity {
	...
	public Slots<Abillity> abilities = new Slots<Abillity>(Abillity.class, 3);
	
	public Slots<Item> armour = new Slots<Item>(Item.class, 1);
	public Slots<Item> holdingItems = new Slots<Item>(Item.class, 2);
	public Slots<Item> bagItems = new Slots<Item>(Item.class, 15);
	...
	public int necessaryXP() { return this.necessaryXP; }
	 ...
	 * @throws IOException
	 */
	public Player(String n, Stats startingStats, String absolutePath) throws JSONException, FileNotFoundException, IOException {
	...
	public void addExp(int exp) {
		...
		while (this.xp >= this.necessaryXP && !this.maxedOut) {
			...
			if(this.level - 1 > this.futureStats.size() - 1) {
				RPG_MAIN.println(this.name + " hat den maximalen Level von " + this.level + " erreicht. Herzlichen Glückwunsch!");
				this.maxedOut = true;
				break;
			}
	...
	/**
	 * Adds the given amount of health to the player while keeping the health in the correct range.
	...
	/**
	 * Sets all stats back to the base stats declared in the beginning. Used for "killing" the player.
	 */
