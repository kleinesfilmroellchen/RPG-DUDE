 ...
 * Capitalizing is standard with such variables.
 * Methods are usually used to return things like ArrayLists and others that can't be expressed in a single constant statement.
...
public final class GameConst {
	/**
	 * Determines fleeing probability in fights. Currently at {@value}.
	 * More precisely, it determines the standard raise of the first random value that "certifies" the second random value.
	 * @see me.rpg.helpers.Functionality#fleeAttempt(double, double) Flee Attempt Function
	 */
	public static final double FLIGHT_P = 0.6d;
	
	/**
	 * Determines with what the defence value of an entity is multiplied when that entity defends on purpose.
	 */
	...
	/**
	 * The absolute Path for wherever the program is run.
	 * Already adds a "\" so a file name can directly be appended to it.
	 */
